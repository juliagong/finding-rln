...
def test_net(net, loader, device, writer, tb_transform, global_step):
    ...
    with tqdm(total=n_val, desc='Test round', unit='batch', leave=False) as pbar:
        for i, batch in enumerate(loader):
            ...
            with torch.no_grad():
                pred = net(img_list)
                mask_pred = pred[0]['masks'][0] if len(pred[0]['masks']) > 0 else torch.zeros_like(true_masks)
...
#            save_image(pred_bin.squeeze(0), os.path.join(dir_outputs, 'f5', 'pred', '{}.png'.format(i)))
