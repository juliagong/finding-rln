...
from eval_nerve import eval_net
import torchvision
from torchvision.models.detection.mask_rcnn import MaskRCNNPredictor
from torchvision.models.detection.faster_rcnn import FastRCNNPredictor

from torch.utils.tensorboard import SummaryWriter
from utils.dataset_nerve import BasicDataset
...
def get_sampler(patient_mapping, splits, mode='train', test_split=5):
    ...
    if mode == 'val':
        ...
        imgs = [f for s in train_splits if s != test_split for f in splits[s]]
...
def train_net(net,
    ...
    for epoch in range(start_epoch, start_epoch + epochs):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{start_epoch + epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                tgt_list = [{'boxes': box.unsqueeze(0), 'labels': torch.tensor([1]).to(device), 'masks': mask} for box, mask in zip(boxes, true_masks)]
 ...
 # unnormalized images for logging
 #                   writer.add_image('images/train', make_grid(torch.stack([tb_transform(img) for img in imgs]), padding=10, nrow=4), global_step)
 # log true and pred masks
 #                   writer.add_image('masks/train/true', make_grid(true_masks, padding=10, nrow=4), global_step)
 #                   writer.add_image('masks/train/pred', make_grid(torch.sigmoid(masks_pred) > 0.5, padding=10, nrow=4), global_step)
