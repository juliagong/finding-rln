            ...
            with torch.no_grad():
                ...
                for j in range(len(pred)):
                    if len(pred[j]['masks']) > 0:
                         accept_idxs = torch.tensor(np.argwhere(pred[j]['scores'].cpu() > 0.5).squeeze(0)).to(device)
                         accept_masks = torch.index_select(pred[j]['masks'], 0, accept_idxs)
                         final_mask = accept_masks.sum(0).squeeze(0)  # all instances in one image
                         mask_pred.append(final_mask)
                    else:
